...
static void PdpLoop(PdpData *pdp) {
	...
	while (true) {
		...
		Wait((loopStartTime += 0.05) - GetClock());			// 50ms Read Interval
	}
}

PdpData::PdpData(RobotLog* robotLog) : m_task("PdpData", (FUNCPTR)PdpLoop) {
	m_robotLog = robotLog;
	m_enabled = false;
	ResetPeaks();

	if (!m_task.Start((int32_t)this)) m_robotLog->LogWrite("PDP Data Failed to Start");
}
...
bool PdpData::IsNewVoltage() {
	bool isNew = m_newVoltage;
	m_newVoltage = false;
	return isNew;
}
