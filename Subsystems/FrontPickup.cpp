...
FrontPickup::FrontPickup(RobotLog* log) : Subsystem("FrontPickup") {
	f_leftArmZeroOffset = 812;
	f_leftArmMaxPosition = 730;
	f_rightArmZeroOffset = 88;     // 112
	f_rightArmMaxPosition = 730;
	f_armTargetDeadband = 15;
	f_incrementValue = 10;
	...
	m_leftArmPot = 	new AnalogInput(AI_FRONT_PICKUP_LEFT_ARM_POT);
	m_leftArmPot->SetAverageBits(2);
	m_leftArmPot->SetOversampleBits(0);

	m_rightArmPot = new AnalogInput(AI_FRONT_PICKUP_RIGHT_ARM_POT);
...
int32_t FrontPickup::GetPosition(Pot pot) {								// GET ARM POSITION
	switch(pot) {														// Arm position relative to offset
		case pLeft:
			return (f_leftArmZeroOffset - (int32_t)((float)m_leftArmPot->GetAverageValue() / 4 + 0.5));
		case pRight:
			return ((int32_t)((float)m_rightArmPot->GetAverageValue() / 4 + 0.5) - f_rightArmZeroOffset);
	}
	return 0;
}
...
void FrontPickup::Periodic() { 											// PERIODIC (Called every periodic loop)
	...
	int32_t curLeftPosition= GetPosition(pLeft);							// Current arm positions
	int32_t curRightPosition = GetPosition(pRight);
	
	int32_t curLeftError = abs(curLeftPosition - m_leftArmTarget);		// Error between position and target
	int32_t curRightError = abs(curRightPosition - m_rightArmTarget);
	...
	if(curLeftError <= 100) {											// Apply I coefficient if near target
		m_leftArmPID->SetPID(0.004, 0.0002, 0.01);						// .0005
	} else {
	...
	if(curRightError <= 100) {											// Apply I coefficent if near target
		m_rightArmPID->SetPID(0.004, 0.0002, 0.01);
	} else {
...
bool FrontPickup::ReadyForShot() {
	if(m_frontMode == fUnknown) return true;
	if(m_frontMode == fShoot)   return OnTarget();
	else SetPickupMode(fShoot); return false;
}

void FrontPickup::SetConstant(std::string key, int32_t value) {
	if(key == "leftArmZeroOffset") {
		f_leftArmZeroOffset = value;
		sprintf(m_log, "FrontPickup: Set LeftArmZeroOffset=%d", value);
		m_robotLog->LogWrite(m_log);
	
	} else if(key == "leftArmMaxPosition") {
		f_leftArmMaxPosition = value;
		sprintf(m_log, "FrontPickup: Set LeftArmMaxPosition=%d", value);
		m_robotLog->LogWrite(m_log);
		
	} else if(key == "rightArmZeroOffset") {
		f_rightArmZeroOffset = value;
		sprintf(m_log, "FrontPickup: Set RightArmZeroOffset=%d", value);
		m_robotLog->LogWrite(m_log);
		
	} else if(key == "rightArmMaxPosition") {
		f_rightArmMaxPosition = value;
		sprintf(m_log, "FrontPickup: Set RightArmMaxPosition=%d", value);
		m_robotLog->LogWrite(m_log);

	} else if(key == "armTargetDeadband") {
...
void FrontPickup::SetPickupMode(FrontMode mode) {						// SET PICKUP MODE
	...
	if (m_frontMode != startMode) {
		sprintf(m_log, "Front Pickup: Mode=%s", GetModeName(mode).c_str());
		m_robotLog->LogWrite(m_log);
...
std::string FrontPickup::GetModeName(FrontMode mode) {
	switch(mode) {
...
int32_t FrontPickup::LimitValue(Pot arm, int32_t position) {				// LIMIT POSITIONS TO WITHIN RANGE
...
void FrontPickup::SetSetpoints(int32_t leftPosition, int32_t rightPosition){	// SET POSITION SETPOINTS
