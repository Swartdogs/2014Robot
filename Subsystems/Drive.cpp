...
Drive::Drive(RobotLog *robotLog) : Subsystem("Drive") {
	...
	m_tapeDetectLED = new Solenoid(1,SOLENOID_TAPE_DETECT);
	
	m_rangefinder = new AnalogInput(AI_RANGEFINDER);
	...
	m_rotatePID = new PIDControl(0.04, 0.005, 0.4); // 0.05, 0.005, 0.4
	m_rotatePID->SetInputRange(-360.0,360.0);
	m_rotatePID->SetOutputRange(-0.7,0.7);
	m_rotatePID->SetSetpoint(0);
	
	m_fireDistance = 0;
	m_startLeftDistance = m_startRightDistance = 0;
	m_endLeftDistance =	m_endRightDistance = 0;
...
void Drive::DriveArcade(float moveValue, float rotateValue) {
	...
	if (moveValue > 0.0) {
		if (rotateValue > 0.0) {
			...
			rightMotorOutput = (moveValue > rotateValue ? moveValue : rotateValue);
		} else {
			leftMotorOutput = (moveValue > -rotateValue ? moveValue : -rotateValue);
...
void Drive::TapeDetectExecute(float move, float rotate) {
	...
	case tArm:
		if(CrossedTape() && !m_tapeDetected) {
			m_tapeDetected = true;
			InitDistance(132, 0.8, true, 24, 0);     // 0.7
			m_tapeMode = tDetect;
			m_tapeDetectLED->Set(true);
			
			sprintf(m_log, "Drive:   Tape Detected: Angle=%5.1f  PWMLeft=%f  PWMRight=%f", GetGyroAngle(), m_leftFront->Get(), m_rightFront->Get());
...
void Drive::SetBrownoutLevel(int level) {
	m_brownoutLevel = level;
}
...
void Drive::SetLEDRing(bool on){
	m_tapeDetectLED->Set(on);
}
