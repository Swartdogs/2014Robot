...
OI::OI(Drive* drive) {
	...
	mcJoystick[0]  = new Joystick(1);
	mcJoystick[1] =  new Joystick(2);
...
//	mcJoy1Button1->WhenPressed(new BackPickupIncrement(BackPickup::aDown));
	mcJoy1Button1->WhenPressed(new BallShooterResetHasBall());
	mcJoy1Button2->WhileHeld(new BackPickupSetMode(BackPickup::bManualPass));
	...
	mcJoy1Button6->WhenPressed(new BackPickupSetMode(BackPickup::bPass));						// ???? Should be while pressed
...
bool OI::GetButtonPress(uint32_t button) {
...
bool OI::UseDriveXaxis() {
	return (driveJoystick->GetRawButton(8) == 1);
}
