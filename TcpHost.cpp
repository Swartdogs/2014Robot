...
static void TcpLoop(TcpHost *host) {
	...
	if ((hostSocket = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {						// Create Host TCP socket
		host->m_robotLog->LogWrite("Dashboard: TCP Socket Error");
		return;
	}
	...
	if (bind(hostSocket, (struct sockaddr *) &addrHost, sizeof(addrHost)) < 0) {			// Bind address to Host Socket
		host->m_robotLog->LogWrite("Dashboard: TCP Binding Error");
		return;
	}

	if (listen(hostSocket, 5) < 0) {														// Open socket to listen
		host->m_robotLog->LogWrite("Dashboard: TCP Listen Failed");
		return;
	}
	...
	while(true) {
		...
		if ((clientSocket = accept(hostSocket, (struct sockaddr *) &addrClient, &clientLen)) >= 0) {
			host->m_robotLog->LogWrite("Dashboard: Connection Accepted");
...
//				printf("Client Message: %s\n", buffer);
				...
				if ((position = clientMesg.find(":")) != std::string::npos) {				// Look for colon at end of command
							...
							if ((position = command.find(",")) != std::string::npos) {		// Look for comma after Put Group (B or V)
								...
								if ((position = command.find(",")) != std::string::npos) {	// Look for comma after Put Index
									...
									if (group == "V") {										// Set Dashboard Value at index
										if (host->SetDashboardValue(index, atoi(command.substr(position + 1).c_str()))) {
											reply += "V," + host->DataString(index);		// Acknowledgment reply
										}
									} else if (group == "B") {								// Set Dashboard Button at index
										if (host->SetDashboardButton(index, atoi(command.substr(position + 1).c_str()))) {
											reply += "B," + host->DataString(index);		// Acknowledgment reply
										}
									}
								}
							}
					...
					if (replySize > 0) {													// Send reply to Dashboard client
						...
						if (send(clientSocket, buffer, replySize, 0) != replySize) host->m_robotLog->LogWrite("Dashboard: TCP Send Error");
					}
				}
			}

			host->m_robotLog->LogWrite("Dashboard: Connection Lost");
		}
	}
}

TcpHost::TcpHost(RobotLog* robotLog, int robotValueCount, int dbValueCount, int dbButtonCount) : m_task("TcpHost", (FUNCPTR)TcpLoop) {
	m_robotLog = robotLog;
	m_robotMode = 0;
	...
	if (!m_task.Start((int32_t)this)) m_robotLog->LogWrite("Dashboard: Host Failed to Start");
}
...
bool TcpHost::GetButton(int group, int button) {
...
bool TcpHost::GetButtonPress(int group, int button) {
	bool vReturn = false;
...
int32_t TcpHost::GetValue(int index) {
	if (index < m_dashboardValueCount) return m_dashboardValue[index];
	return 0;
}

bool TcpHost::SetDashboardButton(int index, int32_t value) {
...
bool TcpHost::SetDashboardValue(int index, int32_t value) {
